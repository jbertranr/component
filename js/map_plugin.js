...
var crs25831 = new L.Proj.CRS('EPSG:25831', '+proj=utm +zone=31 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs', {
  ...
  this.planejamentRefos = L.tileLayer.wms (
    'http://serveisweb.mataro.cat/cartografia/serveisMP/service?',
    {
  ...
  this.capaInicial = L.tileLayer(
    ' http://serveisweb.mataro.cat/mapproxy/tms/1.0.0/ly_guiaurbana_tms/grd_4326_tms/{z}/{x}/{-y}.png', {
      maxZoom: 7,
      minZoom: 0,
      attribution: 'Mataró ',
    }
  ...
  this.baseMaps = {
    'Mapa Inicial':this.capaInicial,
    'Satèl·lit': this.serveiOrtoCache,
    'Planejament':this.planejamentRefos
  };
  ...
  this.queryFeatureGroup = L.markerClusterGroup ({  //Marker Cluster
    maxClusterRadius:35
  //iconCreateFunction: function(cluster) {
		//return L.divIcon({ html: '<b>' + cluster.getChildCount() + '</b>' })
  //                                                  }
  }); // Features resultat de la query
 ...
 //   capaInicial : L.tileLayer(
 //     ' http://serveisweb.mataro.cat/mapproxy/tms/1.0.0/ly_cache_src_guia_urbana_municipal_qgis_etrs89_wmts/grd_25831_tms/{z}/{x}/{-y}.png', {
 //       maxZoom: 7,
 //       minZoom: 0,
 //       attribution: 'Mataró ',
 //     }
...
MapJS.prototype.init = function (options) {
  ...
  this.queryFeatureGroup.addTo(this.map);  
    
  /*this.queryFeatureGroup.on('clustermouseover', function(c) {
 ...
 // this.mapControl = L.control.layers(this.baseMaps, this.overlayMaps);
 ...
 // this.mapControl.addTo(this.map);
  ...
  this.baseLayers = L.control.layers({
    'Capa Inicial2': this.options.capaInicial,
    'Ortofoto': this.serveiOrtoCache,
    'Refós Planejament': this.planejamentRefos,
  });
  
  this.overlayLayers = L.control.layers({
    'Capa Inicial2': this.options.capaInicial,
    'Ortofoto': this.serveiOrtoCache,
    'Refós Planejament': this.planejamentRefos,
  });
  ...
  */
  ...
  this.map.on('dblclick', function(e) {
   // alert(e.latlng);
    ...
    apex
    .item(
      mapCatalog.getMapJS(e.sourceTarget._container.id).options.event.apex_itemDblClick
    )
    .setValue(
...
MapJS.prototype.clearSearchWKT = function (WKT) {
  this.searchFeatureGroup.clearLayers(); 
};
...
MapJS.prototype.searchWKT = function (WKT, HTML = '') {
  if (WKT.length > 10) {
    if (HTML.length > 1) {
       this.searchFeatureGroup.addLayer(omnivore.wkt.parse(WKT).bindPopup(HTML,{maxWidth:600}));
    }
    else{
      this.searchFeatureGroup.addLayer(omnivore.wkt.parse(WKT));
    }
  } 
};

MapJS.prototype.viewWKT = function (WKT,HTML = '') {
  if (WKT.length > 10) {
    if (HTML.length > 1) {
       this.viewFeatureGroup.addLayer(omnivore.wkt.parse(WKT).bindPopup(HTML,{maxWidth:600}));
    }
    else{
      this.viewFeatureGroup.addLayer(omnivore.wkt.parse(WKT).bindPopup(HTML,{maxWidth:600}));
    }
...
MapJS.prototype.queryWKT = function (WKT, HTML = '') {
	
	// omnivore.wkt.parse(WKT).on('ready',function(){alert('ss')});
	
	// var geojsonLayer = L.geoJson(parse(WKT));
    if (WKT.length > 10) {
    //  if (HTML.length > 1) {
    //    this.test = omnivore.wkt.parse(WKT);
	   
	this.actLayer = omnivore.wkt.parse(WKT);
    this.actLayer.on('click',function(e){  // Es força a mostrar l'etiqueta, ja que directament no la mostra
		
		var popup = L.popup({ offset: [0, -35] }).setLatLng(e.latlng).setContent(HTML).openOn(e.propagatedFrom._map); 
		//if (typeof e.propagatedFrom._map !== "undefined" &&  e.propagatedFrom._map !== null ){ 		
			e.propagatedFrom._map.on( 'dragstart zoomend' , function(){
				if (typeof e.propagatedFrom._map !== "undefined" &&  e.propagatedFrom._map !== null ){ 		
					e.propagatedFrom._map.closePopup(); 
				}
			});
	    //}
	});
	
    this.queryFeatureGroup.addLayer(this.actLayer);
	
	
	// var layery = omnivore.wkt.parse(WKT);
	// this.queryFeatureGroup.addLayer(geojsonLayer);
	// layery._popup.setContent('something else')
    // layery.setPopupContent('sfsfd');	   	   
	// this.test.bindPopup('sss');
    }
    // else{
    // this.queryFeatureGroup.addLayer(omnivore.wkt.parse(WKT).bindPopup('asa'));
    //}
   
};


MapJS.prototype.clearQueryWKT = function (WKT) {
